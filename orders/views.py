...
class OrderBookView(View):
    ...
    def write_order(self,data,order_status):
        try:
            obj, created = Order.objects.update_or_create(
                    ...
                    bookoption_id_id = data['book_option'],
    ...
    def post(self, request):
        try :
            ...
            user_wallet = UserWallet.objects.filter(user_id_id = data['buyer']).get()
    ...
    def delete(self, request, order_id):
        ...
        try :
            ...
            user_library = UserLibrary.objects.filter(
                user_id_id = order.user, 
                book_id_id = order.bookoption_id.book.id)
            ...
            if user_library.get().bookmark == 0 and not user_library.get().deleted_at :
                ...
                user_wallet = UserWallet.objects.filter(user_id_id = order.user).get()
...
class OrderHistoryView(View):
    ...
    def get(self, request,**kargs):
        ...
        ##order_id 없으면 -> 리스트 페이지 +상세 페이지
        ##order_id 있으면 주문 상세 내역 페이지
        return 0